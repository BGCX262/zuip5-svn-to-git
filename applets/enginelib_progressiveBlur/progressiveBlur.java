import processing.core.*; import noc.*; import enginelib.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; import javax.sound.midi.*; import javax.sound.midi.spi.*; import javax.sound.sampled.*; import javax.sound.sampled.spi.*; import java.util.regex.*; import javax.xml.parsers.*; import javax.xml.transform.*; import javax.xml.transform.dom.*; import javax.xml.transform.sax.*; import javax.xml.transform.stream.*; import org.xml.sax.*; import org.xml.sax.ext.*; import org.xml.sax.helpers.*; public class progressiveBlur extends PApplet { // Vector 3D class from nature of code http://www.shiffman.net/teaching/nature/library/
...
public void setup() {
  size(160,120,P3D); // sorry for this eyekiller, but its because the performance
  oc3d = new objectControler3D(this);
  oc3d.addObject( new Camera3D("Cam1") );
    
  Object3DBlur tower = new Object3DBlur("Tower1");
...
class Object3DBlur extends Object3D {
  
  Object3DBlur(String sname) {
    super(sname);
  }
  
  public void objectquicksort() {
    objectquicksort(0,objects.length-1);
  }
  
  public void objectquicksort(int left, int right) {
    if(left < right) {
      int teiler = teile(left,right);
      objectquicksort(left,teiler-1);
      objectquicksort(teiler+1,right);
    }
  }
  
  public float date(int pos) {
    Vector3D v = objects[pos].position;
  ...
  public int teile(int left, int right) {
    ...
    float pivot = date(right);
    do {
      while(date(i) <= pivot && i < right) {i+=1;}
      while(date(j) >= pivot && j > left)  {j-=1;}
      if(i < j) {
        Object3D dummy = objects[i];
        objects[i] = objects[j];
        objects[j] = dummy;
      }
    } while(i < j);
    Object3D dummy = objects[i];
    objects[i] = objects[right];
    objects[right] = dummy;
    return i;
  }
  ...
  public void postdraw() {
    ...
    for( int i = length-1 ; i >= 0 ; i -= 1 ) {
      objects[i].display();
      if(i%BLURINTERVAL == 0) {
        graphic.filter(BLUR, abs(date(i)));
      }
    }
    graphic.popMatrix();
  }
...
class Cube3D extends Object3D {
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "progressiveBlur" });  }}